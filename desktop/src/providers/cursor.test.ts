...
test('a follow-up reuses its checkpoint when the first response included reasoning', async () => {
  const requests: FakeRequest[] = [];
  const checkpoint = encodeBytesField(8, new Uint8Array([7, 8, 9]));
  const connect = (() => {
    const request = new FakeRequest();
    requests.push(request);
    const session = Object.assign(new EventEmitter(), {
    ...
    queueMicrotask(() => {
      request.emit('response', { ':status': 200 });
      request.emit('data', serverFrame(encodeMessageField(3, checkpoint)));
      request.emit('data', interactionUpdate(encodeMessageField(4, encodeStringField(1, 'private reasoning'))));
      request.emit('data', interactionUpdate(encodeMessageField(1, encodeStringField(1, 'Visible answer.'))));
      request.emit('data', interactionUpdate(encodeMessageField(14, new Uint8Array(0))));
    });
    return session;
  }) as never;
  ...
  const provider = new CursorProvider({ apiKey: 'token', connectImpl: connect });
  await collect(provider.chatCompletionStream(request([{ role: 'user', content: 'First prompt' }])));
  await collect(provider.chatCompletionStream(request([
    { role: 'user', content: 'First prompt' },
    // The OpenAI-compatible transcript carries visible messages, not Cursor's
    // private reasoning stream.
  ...
  assert.deepEqual(
    secondState,
    decodeMessage(checkpoint),
    'private reasoning must not prevent matching the visible transcript to its checkpoint',
  );
});
